...
class controlador_org_empresa extends empresas {
    ...
    public function registros_patronales_r(int $org_empresa_id){
        ...
        $filtro['org_tipo_sucursal.es_matriz'] = 'activo';
        $r_org_sucursal = (new org_sucursal($this->link))->filtro_and(filtro: $filtro);
        if(errores::$error){
            return $this->errores->error(mensaje: 'Error al obtener sucursales', data: $r_org_sucursal);
        }

        $filtro_reg_pat['org_sucursal.id'] = $r_org_sucursal->registros_obj[0]->org_sucursal_id;
        $r_im_registro_patronal = (new im_registro_patronal($this->link))->filtro_and(filtro: $filtro_reg_pat);
        if(errores::$error){

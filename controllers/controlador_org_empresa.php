...
class controlador_org_empresa extends system {
    ...
    public function alta(bool $header, bool $ws = false): array|string
    {
        ...
        $in_exterior = (new org_empresa_html())->input_exterior(cols: 12,row_upd:  new stdClass(),value_vacio:  true);
        if(errores::$error){
            $error = $this->errores->error(mensaje: 'Error al generar input',data:  $in_exterior);
            print_r($error);
            die('Error');
        }
        $this->inputs->exterior = $in_exterior;


        $fec_fecha_inicio_operaciones = (new org_empresa_html())->fec_fecha_inicio_operaciones(cols: 6,row_upd:
        ...
        $fec_fecha_ultimo_cambio_sat = (new org_empresa_html())->fec_fecha_ultimo_cambio_sat(cols: 6,row_upd:
            new stdClass(),value_vacio:  true);
        if(errores::$error){
            $error = $this->errores->error(mensaje: 'Error al generar input',data:  $fec_fecha_ultimo_cambio_sat);
            print_r($error);
            die('Error');
        }
        $this->inputs->fecha_ultimo_cambio_sat = $fec_fecha_ultimo_cambio_sat;
